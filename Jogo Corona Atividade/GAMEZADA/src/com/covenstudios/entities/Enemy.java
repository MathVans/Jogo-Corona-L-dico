...
public class Enemy extends Entity{
	...
	public Enemy(int x, int y, int width, int height, BufferedImage sprite) {
		...
		dir = right_dir;
		for(int i=0; i<=3; i++) {
		spritesdir[i] = Game.playerspritesheet.getSprite(0+(i*32), 66,32, 32);
		}
		damagedEnemy = Game.playerspritesheet.getSprite(288, 66, 32, 32);
	
	for(int i=0; i<=3; i++) {
		spritesesq[i] = Game.playerspritesheet.getSprite(160+(i*32), 66,32, 32);
		}
